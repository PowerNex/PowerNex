...
version (Windows) version = with_libc; // no reason not to, Windows will always have it anyway
...
version (bare_metal) __gshared multiboot_info* bootInfo;
...
class TypeInfo_Class : TypeInfo {
	...
	override @property immutable(MoreTypeInfo*) rtInfo() nothrow pure @safe const {
		return m_RTInfo;
	}
...
version (deh2) {
	...
	FuncTable* __eh_finddata(void* address) {
		...
		Lagain:
             ...
             */
				if ((cast(ubyte*)fptr)[0] == 0xE9) { // JMP target = RIP of next instruction + signed 32 bit displacement
 ...
 */
	...
	version (without_exceptions) {
		...
		while (1) // for each function on the stack
		{
             ...
             * need to follow the JMP.
             */
				if ((cast(ubyte*)funcoffset)[0] == 0xE9) { // JMP target = RIP of next instruction + signed 32 bit displacement
...
version (without_moduleinfo) {
	...
	struct ModuleInfo {
		...
		version (all) {
			deprecated("ModuleInfo cannot be copy-assigned because it is a variable-sized struct.") void opAssign(in ModuleInfo m) {
				_flags = m._flags;
				_index = m._index;
			}
		} else {
